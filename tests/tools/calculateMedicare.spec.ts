...
describe('calculateMedicareCosts', () => {
  ...
  describe('2024 base calculations', () => {
    it('should calculate correct base premium for single filer below first threshold', () => {
      expect(calculateMedicareCosts(100000, "single", 0)).toBe(2088); // $174 * 12
    });

    it('should calculate correct premium for single filer in second bracket', () => {
      expect(calculateMedicareCosts(120000, "single", 0)).toBe(2928); // $244 * 12
    });

    it('should calculate correct premium for married filer below first threshold', () => {
      expect(calculateMedicareCosts(200000, "married", 0)).toBe(2088); // $174 * 12
    });

    it('should calculate correct premium for married filer in second bracket', () => {
      expect(calculateMedicareCosts(220000, "married", 0)).toBe(2928); // $244 * 12
    });
  });
  ...
  describe('bracket transitions', () => {
    ...
    it('should handle single filer bracket transitions correctly', () => {
      ...
      expect(calculateMedicareCosts(103000, "single", 0)).toBe(2928); // Second bracket
      expect(calculateMedicareCosts(129000, "single", 0)).toBe(4188); // Third bracket
      expect(calculateMedicareCosts(161000, "single", 0)).toBe(5028); // Fourth bracket
      expect(calculateMedicareCosts(193000, "single", 0)).toBe(5940); // Fifth bracket
      expect(calculateMedicareCosts(500000, "single", 0)).toBe(6780); // Sixth bracket
    });

    // Married filer brackets
    it('should handle married filer bracket transitions correctly', () => {
      // Test exact bracket boundaries
      expect(calculateMedicareCosts(206000, "married", 0)).toBe(2928); // Second bracket
      expect(calculateMedicareCosts(258000, "married", 0)).toBe(4188); // Third bracket
      expect(calculateMedicareCosts(322000, "married", 0)).toBe(5028); // Fourth bracket
      expect(calculateMedicareCosts(386000, "married", 0)).toBe(5940); // Fifth bracket
      expect(calculateMedicareCosts(750000, "married", 0)).toBe(6780); // Sixth bracket
    });
  });
  ...
  describe('inflation adjustments', () => {
    it('should calculate inflation correctly after 5 years', () => {
      const baseAmount = calculateMedicareCosts(100000, "single", 0);
      const inflatedAmount = calculateMedicareCosts(100000, "single", 5);
    ...
    it('should adjust brackets for inflation', () => {
      ...
      expect(calculateMedicareCosts(year5Income - 1, "single", 5))
        .toBeLessThan(calculateMedicareCosts(year5Income + 1, "single", 5));
    });
  ...
  describe('edge cases', () => {
    it('should handle zero income', () => {
      expect(calculateMedicareCosts(0, "single", 0)).toBe(2088);
      expect(calculateMedicareCosts(0, "married", 0)).toBe(2088);
    });

    it('should handle very high incomes', () => {
      expect(calculateMedicareCosts(1000000, "single", 0)).toBe(6780);
      expect(calculateMedicareCosts(1000000, "married", 0)).toBe(6780);
    });

    it('should handle negative years', () => {
      expect(calculateMedicareCosts(100000, "single", -1)).toBe(2052);
    });

    it('should handle negative income', () => {
      expect(calculateMedicareCosts(-50000, "single", 0)).toBe(2088);
    });
  });
  ...
  describe('rounding behavior', () => {
    it('should always return whole numbers', () => {
      const amount = calculateMedicareCosts(100000, "single", 3.5);
